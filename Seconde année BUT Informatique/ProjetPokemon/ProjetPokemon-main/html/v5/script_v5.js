document.addEventListener("DOMContentLoaded", function() {
    ...
    function displayPokemon(pokemonData) {
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            if (pokemon.form === "Normal") {
                ...
                // Filtrage et traitement des générations
                ...
                for (const genDataKey in generation) {
                    ...
                    genData.forEach(genNum => {
                        if (genNum.id === pokemon.pokemon_id) {
                            if (genNum.generation_number) { // Vérifier si la valeur de la génération est définie
                ...
                newRow.innerHTML = `
                    ...
                    <td>${generations.length > 0 ? generations.join(', ') : 'Not implemented'}</td>
                    <td>${types.join(', ')}</td>
    ...
    function displayPokemonDetails(pokemon, types, generations) {
        ...
        let detailsHTML = `
            ...
            <p>Génération : ${generations.length > 0 ? generations.join(', ') : 'Not implemented'}</p>
    ...
    function filterPokemon() {
        ...
        // Trier les Pokémon selon la colonne actuelle et la direction de tri actuelle
        sortPokemon(currentSortColumn);
    ...
    document.querySelectorAll('th').forEach(headerCell => {
        headerCell.addEventListener('click', function() {
            ...
            sortPokemon(columnName);
    
            // Réinitialiser le style de tous les en-têtes de colonne
            document.querySelectorAll('th').forEach(cell => {
                cell.style.fontWeight = 'normal'; // Réinitialiser à la normale
            });
    
            // Mettre en gras l'en-tête de colonne cliqué
            this.style.fontWeight = 'bold';
        });
    });
    
    
    function sortPokemon(columnName) {
        const sortDirection = (currentSortColumn === columnName && currentSortDirection === 1) ? -1 : 1;
        ...
        sortedPokemon.sort((a, b) => {
            ...
            if (columnName === 'types') {
                // Extract the first type of each Pokémon for sorting
                valueA = getPokemonFirstType(a);
                valueB = getPokemonFirstType(b);
            } else {
                valueA = a[columnName];
                valueB = b[columnName];
    
                // Handle sorting by generation as an array
                if (columnName === 'generation') {
                    // Convertir les valeurs en tableau si elles ne le sont pas déjà
                    valueA = Array.isArray(valueA) ? valueA : [];
                    valueB = Array.isArray(valueB) ? valueB : [];
    
                    // Accéder au premier élément du tableau s'il existe
                    valueA = valueA.length > 0 ? parseInt(valueA[0]) : undefined;
                    valueB = valueB.length > 0 ? parseInt(valueB[0]) : undefined;
                }
    
                // En cas d'égalité de valeur, départager sur le nom
                if (valueA === valueB) {
                    return a['pokemon_name'].localeCompare(b['pokemon_name']) * sortDirection;
                }
            }
            ...
            if (!isNaN(valueA) && !isNaN(valueB)) { // If values are numbers
                return (valueA - valueB) * sortDirection;
            } else { // If values are strings
    ...
    // Function to extract the first type of a Pokémon
    function getPokemonFirstType(pokemon) {
        // Check if the pokemon_type data is loaded
        if (typeof pokemon_type !== 'undefined') {
            const pokemonTypeData = pokemon_type.find(typeData => typeData.pokemon_id === pokemon.pokemon_id);
            if (pokemonTypeData && pokemonTypeData.type.length > 0) {
                return pokemonTypeData.type[0]; // Return the first type
            }
        }
        return undefined;
    }
